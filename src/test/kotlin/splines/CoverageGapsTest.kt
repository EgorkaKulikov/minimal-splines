...
class CoverageGapsTest {
    ...
    fun deBoorFix_orderOutsideRangeRejected() {
        for (r in listOf(-1, 3)) {
            ...
            assertTrue(message(e).contains("r must be in {0,1,2}") && message(e).contains("got $r"), message(e))
        }
        for (r in listOf(0, 3)) {
            val e = assertFailsWith<IllegalArgumentException>("DiscreteDeBoorFix r=$r") { DiscreteDeBoorFixFunctionals(basis, r = r) }
            assertTrue(message(e).contains("r must be in {1,2}") && message(e).contains("got $r"), message(e))
        }
    }
    ...
    fun deBoorFixZero_inflectionNodeRejected() {
        ...
        assertTrue(message(e).contains("degenerate rho'sigma''-rho''sigma'"), message(e))
    }
}
