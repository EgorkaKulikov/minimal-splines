...
/**
 * Reading and writing golden references without external dependencies.
 *
 * The format is JSON; every `Double` is written as a 16-character hex string (`toRawBits()`),
 * so values are restored bit for bit. Supported types: object (`Map<String, Any?>`),
...
object GoldenIo {
    ...
    /** Extracts a nested object by key, failing with a clear message if the key is missing. */
    fun section(root: Map<String, Any?>, key: String): Map<String, Any?> =
        obj(root[key] ?: fail("The golden reference has no section '$key'"))

    fun entry(section: Map<String, Any?>, key: String): Any? {
        if (!section.containsKey(key)) fail("The golden reference has no case '$key'; run ./gradlew regenerateGolden")
        return section[key]
    }
    ...
    /**
     * `‖got − expected‖∞ / max(‖expected‖∞, 1) ≤ tol`. Elements that match bit for bit
    ...
    fun assertClose(expected: Array<DoubleArray>, got: Array<DoubleArray>, tol: Double, label: String) {
        assertEquals(expected.size, got.size, "$label: number of rows")
        for (i in expected.indices) {
    ...
    /**
     * Compares an inversion residual as rounding noise rather than as a measurement:
     * it is accepted if both values are at most 1e-12, or are of the same order of magnitude
     ...
     * the magnitude depends on the order of operations in a particular LAPACK implementation.
     */
    ...
    /** Bit-for-bit match (`toBits()`: all NaNs are considered equal to each other). */
    ...
    fun assertBitsOrNull(expected: Double?, got: Double?, label: String) {
        if (expected == null || got == null) {
            assertEquals(expected, got, "$label: null match")
        } else {
    ...
    private fun writeValue(sb: StringBuilder, v: Any?, level: Int) {
        when (v) {
            ...
            is List<*> -> {
                ...
                // A list of scalars goes on one line, a list of containers goes line by line.
