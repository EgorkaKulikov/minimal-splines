...
Чем они проверены вместо этого: средством **И** (биортогональность `chi_j(omega_i) = delta_{ij}`,
идемпотентность проектора, точность на `span`) и средством **Н/С** по СЛЕДСТВИЯМ:
слой L6b сверяет итоговые `E_h` схем и наблюдаемый порядок сходимости с независимым
Nystrom, а `M2`/`d` входят в вычисление этих `E_h` (схема Слоана) — ошибка в них сдвинула
бы порядок сходимости `sloan` с наблюдаемых 4.2...4.3 к базовым 3, что L6b ловит.
