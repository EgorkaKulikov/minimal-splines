...
| Схема Кулкарни для квазиинтерполянтов (`mu`, `lambda`) | `kulkarniQuasi` | — | **Без источника**: теория Кулкарни существенно требует `P^2 = P`; статус — численное наблюдение |
...
| Сведение Вольтерры I → II рода дифференцированием | `solvers/volterra`, `FirstKindSolver` | [Wazwaz 2011b], [Brunner 2004] | Подтверждено для случая `m = 1` |
| Формулы `(Vu)'` и `(Vu)''` (правило Лейбница) | `VolterraOperator.applyDeriv`, `applyDeriv2` | [Makarov, Kulikov 2026] | `(Vu)'` подтверждено; `(Vu)''` — вывод проверен вручную, отдельной публикации нет |
